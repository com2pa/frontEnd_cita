...
export default function ContactFormWithSocialButtons() {
  ...
  // mostrando los empleado que ofrencen los servicios
  useEffect(()=>{
  ...
  const handleNewPatient = async()=>{
    ...
    try {
      // console.log(name, email, phone,date,time,serviceSelected);
      const{data}=await axios.post('/api/customer',{name, email, phone,date,time,serviceSelected});
...
useEffect(() => {
  ...
  const isValidTime = () => {
    const isWithinTimeRange = REGEX_TIME.test(time) && time >= "08:00" && time <= "17:30";
    ...
    // Permitir días válidos (lunes a domingo)
    const isValidDay = selectedDay >= 0 && selectedDay <= 6;

    return isWithinTimeRange && isValidDay;
  };

  const isValid = isValidTime();
  setTimeValidation(isValid);
  ...
  if (!hasShownToast) {
    toast({
      ...
      description: isValid
        ? 'Horario válido: entre las 8:00 a.m. y las 5:30 p.m., de lunes a domingo.'
        : 'Horario inválido.antes 8:00 a.m. y las 5:30 p.m.',
      status: isValid ? 'success' : 'error',
      duration: 2000,
      isClosable: true,
    });
    hasShownToast = true; // Marca que el toast ya se mostró
  }
}, [time, date, toast]);
                    ...
                    <FormControl >
                      ...
                      <InputGroup borderColor="#E0E1E7" size="sm">
                        ...
                        <Select placeholder="Select option" onChange={({ target }) => setServiceSelected(target.value)}>
                          {empleado.map((service)=>{
                            return <option key={service.id} value={service.id}>${service.precio} -{service.service.NameService} - {service.user.name}</option>;
